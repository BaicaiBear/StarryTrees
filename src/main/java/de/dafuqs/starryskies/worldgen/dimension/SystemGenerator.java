...
public class SystemGenerator {
    ...
    public static final Codec<SystemGenerator> CODEC = RecordCodecBuilder.create(
            instance -> instance.group(
                    ...
                    Codec.INT.fieldOf("min_distance_between_spheres")
                            .forGetter(generator -> generator.minDistanceBetweenSpheres),
                    ...
                    DefaultSphere.CODEC.listOf().fieldOf("fixed_spheres")
                            .forGetter(generator -> generator.defaultSpheres))
    ...
    public SystemGenerator(int spheresPerSystem, int minDistanceBetweenSpheres, int floorHeight, BlockState floorState,
            BlockState bottomState, List<DefaultSphere> defaultSpheres) {
    ...
    public System getSystem(WorldAccess worldAccess, long seed, Point systemPos) {
        ...
        if (system == null) {
            ...
            system = System.generateSystem(this, worldAccess.getBottomY(), worldAccess.getHeight(), seed, systemPos,
                    worldAccess.getRegistryManager());
            systemCache.put(systemPos, system);
        }
    ...
    public Iterable<? extends PlacedSphere<?>> getSystem(Chunk chunk, long seed,
    ...
    public record System(List<PlacedSphere<?>> spheres) implements Iterable<PlacedSphere<?>> {
        ...
        private static System generateSystem(SystemGenerator systemGenerator, int bottomY, int worldHeight, long seed,
                @NotNull Point systemPoint, DynamicRegistryManager registryManager) {
            ...
            List<PlacedSphere<?>> defaultSpheres = getDefaultSpheres(systemGenerator, systemPointX, systemPointZ,
                    systemRandom, registryManager);
            ...
            for (int currentDensity = 0; currentDensity < systemGenerator.spheresPerSystem; currentDensity++) {
                ...
                // create new planets
                @Nullable
                PlacedSphere<?> currentSphere = getRandomSphere(systemGenerator, systemRandom, systemPoint,
                        registryManager, bottomY, worldHeight, spheresInSystem);
            ...
            StarrySkies.LOGGER.debug("Created a new system with {} spheres at system position {},{}",
                    spheresInSystem.size(), systemPointX, systemPointZ);
        ...
        private static List<PlacedSphere<?>> getDefaultSpheres(SystemGenerator systemGenerator, int systemPointX,
                int systemPointZ, ChunkRandom systemRandom, DynamicRegistryManager registryManager) {
            if (systemGenerator.defaultSpheres.isEmpty()) {
                return List.of();
            }

            Registry<ConfiguredSphere<?, ?>> templateRegistry = registryManager
            ...
            for (DefaultSphere defaultSphere : systemGenerator.defaultSpheres) {
                if (systemPointX == defaultSphere.systemX && systemPointZ == defaultSphere.systemZ) {
                    ...
                    if (template != null) {
                        ...
                        PlacedSphere<?> sphere = template.generate(systemRandom, registryManager, pos,
                                template.getSize(systemRandom));
        ...
        private static @Nullable PlacedSphere<?> getRandomSphere(SystemGenerator systemGenerator,
                ChunkRandom systemRandom, @NotNull Point systemPoint, DynamicRegistryManager registryManager,
                int bottomY, int worldHeight, List<PlacedSphere<?>> spheresInSystem) {
            ...
            do {
                do {
                    Map<ConfiguredSphere<?, ?>, Float> spheresInSelectedGroup = getWeightedRandom(
                            systemGenerator.generationGroups, systemRandom);
                ...
                StarrySkies.LOGGER.debug("Created a new sphere of type {} Next random: {}", selectedSphere,
                        systemRandom.nextInt());
                ...
                int yPos = bottomY + systemGenerator.floorHeight + iRadius
                        + systemRandom.nextInt(((worldHeight - iRadius * 2 - systemGenerator.floorHeight)));
