...
public class ShellSphere<SC extends ShellSphere.Config> extends Sphere<SC> {
	...
	@Override
	public PlacedSphere<?> generate(ConfiguredSphere<? extends Sphere<SC>, SC> configuredSphere, SC config,
			ChunkRandom random, DynamicRegistryManager registryManager, BlockPos pos, float radius) {
		return new ShellSphere.Placed<>(configuredSphere, radius, configuredSphere.getDecorators(random),
				configuredSphere.getSpawns(random), random, config.innerBlock.getForSphere(random, pos),
				config.shellBlock.getForSphere(random, pos), config.shellThickness.get(random));
	}
	...
	public static class Config extends SphereConfig {
		...
		public static final Codec<ShellSphere.Config> CODEC = RecordCodecBuilder.create((instance) -> instance.group(
				...
				.apply(instance,
						(sphereConfig, innerBlock, shellBlock, shellThickness) -> new Config(sphereConfig.size,
								sphereConfig.decorators, sphereConfig.spawns, sphereConfig.generation, innerBlock,
								shellBlock, shellThickness)));
	...
	public static class Placed<SC extends SphereConfig> extends PlacedSphere<SC> {
		...
		public Placed(ConfiguredSphere<? extends Sphere<SC>, SC> configuredSphere, float radius,
				List<RegistryEntry<ConfiguredSphereDecorator<?, ?>>> decorators,
				List<Pair<EntityType<?>, Integer>> spawns, ChunkRandom random, BlockStateProvider innerBlock,
				BlockStateProvider shellBlock, int shellRadius) {
