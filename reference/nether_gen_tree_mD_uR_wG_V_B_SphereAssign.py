...
MIN_DIST = 40
NUM_ROOTS = 1
HEIGHT_MIN, HEIGHT_MAX = 40, 88
ROOT_H_MIN, ROOT_H_MAX = 30, 100
...
def generate_dummy_configs():
    ...
    common_types = [
        {"id": "minecraft:netherrack", "size": [10, 30]},
        {"id": "minecraft:basalt", "size": [5, 15]},
        {"id": "minecraft:blackstone", "size": [8, 20]},
        {"id": "minecraft:soul_sand", "size": [5, 10]},
        {"id": "minecraft:glowstone", "size": [2, 5]}
    ]
    ...
    for biome in BIOME_DATA:
        ...
        if not os.path.exists(fname):
            ...
            for i, t in enumerate(common_types):
                ...
                if i == len(common_types) - 1: w = leftover_prob
...
def assign_spheres(valid_indices, biome_indices, heights, config_map, seed):
    ...
    for idx_in_valid, pt_idx in enumerate(valid_indices):
        ...
        if cfg:
            ...
            assigned_types[idx_in_valid] = "minecraft:air"
...
def calculate_biomes_nether_style(temps, humidities):
    num_points = len(temps)
    min_sq_distances = np.full(num_points, np.inf)
    best_biome_indices = np.zeros(num_points, dtype=int)
    
    print("Calculating Biomes (Nether Style)...")
    
    for i, biome in enumerate(BIOME_DATA):
        m = np.array([biome['t'][0], biome['h'][0]])
        n = np.array([biome['t'][1], biome['h'][1]])
        offset = biome['offset']
        
        v = np.vstack((temps, humidities)).T
        
        dist_lower = np.maximum(0, m - v)
        dist_upper = np.maximum(0, v - n)
        
        component_distances = dist_lower + dist_upper
        sum_sq_dist = np.sum(component_distances ** 2, axis=1)
        
        total_metric = (offset ** 2) + sum_sq_dist
        
        mask = total_metric < min_sq_distances
        min_sq_distances[mask] = total_metric[mask]
...
def calc_heights(points, roots, parent, mask, seed):
    ...
    heights = np.full(len(points), HEIGHT_MIN - 5.0)
...
def plot_sphere_map(points, radii, types, biome_indices, filename="map_spheres.png"):
    ...
    ax.set_aspect('equal')
    ax.axis('off')
    ax.set_title("Nether Biome Spheres", fontsize=16)
...
def main():
    ...
    # 3. Noise Generation (UPDATED SCALING)
    norm_x = (valid_points[:, 0] + RADIUS) / (2 * RADIUS) * NOISE_SCALE
    norm_y = (valid_points[:, 1] + RADIUS) / (2 * RADIUS) * NOISE_SCALE
    
    # Generate Temp mapped to [-2.22, 2.22]
    raw_temp = fractal_noise(norm_x, norm_y, seed=RANDOM_SEED+101, octaves=2)
    # Using interpolation to map from approx noise range [-1, 1] to target range
    temperature = np.interp(raw_temp, [-1.0, 1.0], [-2.22, 2.22])
    
    # Generate Humidity mapped to [-1.69, 1.69]
    raw_hum = fractal_noise(norm_x + 12.0, norm_y - 12.0, seed=RANDOM_SEED+202, octaves=3)
    humidity = np.interp(raw_hum, [-1.0, 1.0], [-1.69, 1.69])
    
    # 4. Biome Determination
    biome_indices = calculate_biomes_nether_style(temperature, humidity)
    
    # 5. Load Configs and Assign Spheres
